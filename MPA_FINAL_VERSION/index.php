...
<body>
    ...
    <div class="section" id="pollsMainPage">
        <h2>Top 5 Most Recent Active Polls</h2>

        <div class="article" id="mainPolls">
        ...
        <ol>
            <?php
                ...
                while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    echo '<li id="pollQuestion">' . $pollQuestion . ' (Date Created: ' . $createdDate . ')</li>';
                }
            ?>
    ...
    <!-- JS Ajax code to update every 90 seconds -->
    <script>
        function updatePolls() {
            var xmlhttp = new XMLHttpRequest();
            xmlhttp.open("POST", "updateMainPage.php", true);
            xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");

            xmlhttp.onreadystatechange = function () {
                if (this.readyState == 4 && this.status == 200) {
                    var obj = JSON.parse(xmlhttp.responseText);
                    var pollId = obj.poll_id;
                    var userId = obj.user_id;
                    var pollQuestion = obj.question;
                    var openDate = obj.open_dt;
                    var createdDate = obj.created_dt;

                    document.getElementById("pollQuestion").innerHTML = pollQuestion + ' (Date Created: ' + createdDate + ')';
                }
            }
            xmlhttp.send();
        }
        setInterval(updatePolls, 9000);
    </script>
